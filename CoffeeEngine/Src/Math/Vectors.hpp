...
namespace CoffeeEngine
{
	namespace Math
	{
		struct Vector3
		{
			...
			Vector3(Vector3&& vector) : _x(vector._x), _y(vector._y), _z(vector._z)
			{
		...
		{
			float _x, _y, _z, _w;
			Vector4() : _x(0.0f), _y(0.0f), _z(0.0f), _w(1.0f) {};
			...
			Vector4(Vector4&& vector) : _x(vector._x), _y(vector._y), _z(vector._z), _w(vector._w)
			{
				vector._x = vector._y = vector._z = 0.0f;
				vector._w = 1.0f;
			};
